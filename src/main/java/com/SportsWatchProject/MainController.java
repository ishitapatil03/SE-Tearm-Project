...
public class MainController {
	...
	public ModelAndView getTeamScores(
		...
		ArrayList<HashMap<String, String>> gameDetails = new ArrayList<HashMap<String, String>>();
		String url = "https://api.mysportsfeeds.com/v1.2/pull/nba/2018-2019-regular/scoreboard.json?fordate=20181105";
...
/*	@GetMapping("/time")
	public ModelAndView getTime(
		...
		ModelAndView time = new ModelAndView("time");
		ArrayList<HashMap<String, String>> gameDetails = new ArrayList<HashMap<String, String>>();
		String url = "http://api.timezonedb.com/v2.1/get-time-zone?key=IRLIESVQOCSF&format=json&by=zone&zone=America/Chicago";
		String encoding = Base64.getEncoder().encodeToString("IRLIESVQOCSF:DCtrue@03".getBytes());
		...
		try {
			...
			System.out.println(str);
			//JsonNode jsonNode1 = actualObj.get("lastUpdatedOn");
	        JsonNode formatted = root.get("result").get("formatted");
		...
		time.addObject("gameDetails", gameDetails);
